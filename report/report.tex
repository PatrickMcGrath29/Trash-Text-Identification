...
\begin{document}
    ...
    \State $\text{initialize w} \gets (0, ..., 0)$
    \State $\text{Monitor Stream...}$
    \State $\text{ingest new document d } \gets (d_1, ..., d_v)$
    \State $\text{Vectorize, apply TF-IDF}$
    \State $\text{Predict classification with d^T w}$
    \State $\text{Observe label for document}$
    \State $\text{Compute Loss L} \gets max(0, 1-y*d^T*w)$
    \State $\text{w_{new}} \gets w + y*L*d$
    \EndProcedure
    \end{algorithmic}
    \end{algorithm}
